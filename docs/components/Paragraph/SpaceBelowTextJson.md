...
Inserts one or more paragraphs and applies spacing below each, producing `insertText` requests followed by `updateParagraphStyle(spaceBelow)` requests.
...
| Space Below | double (tree) | No | 0.0 | Space below the paragraph, in points, index-matched to Text (a shorter list repeats its last value; flattened the same way as Text). Only applied where >0. |
