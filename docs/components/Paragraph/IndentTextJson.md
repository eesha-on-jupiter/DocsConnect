...
Inserts one or more paragraphs and applies a start indent to each, producing `insertText` requests followed by `updateParagraphStyle(indentStart)` requests.
...
| Indent | double (tree) | No | 18.0 | Start indent, in points, index-matched to Text (a shorter list repeats its last value; flattened the same way as Text). Only applied where >0 — a value of 0 or less is skipped. |
